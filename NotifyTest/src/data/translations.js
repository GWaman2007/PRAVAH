export const SUPPORTED_LANGUAGES = [
  ...
  },
  {
    ...
    isUnicode: true,
    ttsLang: 'mn-IN',
    ttsGoogleLang: 'bn', // Reads Manipuri in Eastern Nagari script phonetically
    voiceFallbacks: ['mn-IN', 'bn-IN', 'hi-IN', 'en-IN'],
  },
];

// Pre-defined high accuracy translations for preset incidents
...
export const CALM_BROADCAST_SCRIPTS = {
  'inc-nh29-kohima': {
...
export function getCalmBroadcastSpeechText(incident, langId, customText) {
  if (incident && CALM_BROADCAST_SCRIPTS[incident.id]?.[langId]) {
    return CALM_BROADCAST_SCRIPTS[incident.id][langId];
  }

  const hw = incident?.highway || 'Highway';
  const loc = incident?.district || incident?.stretch || 'Sector';
  const disruption = incident?.disruptionType || 'obstruction';
  const detour = incident?.detourRoute || 'alternate bypass';
...
export function generateLocalizedAlerts(incident, preferMeiteiMayek = true) {
  if (PRESET_TRANSLATIONS[incident.id]) {
    const p = PRESET_TRANSLATIONS[incident.id];
    return {
      en: p.en,
      hi: p.hi,
      as: p.as,
  ...
  const loc = incident.district || incident.stretch || 'NER Sector';
  const disruption = incident.disruptionType || 'Obstruction';
  const detour = incident.detourRoute || 'Designated Alternate Bypass';
...
export function calculateSmsMetrics(text, isUnicode = false) {
