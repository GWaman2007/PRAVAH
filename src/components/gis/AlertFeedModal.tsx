...
export const AlertFeedModal: React.FC<AlertFeedModalProps> = ({
  ...
  const getSeverityIcon = (severity: string) => {
    if (severity === 'CRITICAL') {
      return <ShieldAlert className="w-4 h-4 text-status-blocked-solid mt-0.5 shrink-0" />;
    }
    if (severity === 'WARNING' || severity === 'HIGH RISK') {
      return <AlertTriangle className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />;
    }
    return <Radio className="w-4 h-4 text-primary mt-0.5 shrink-0" />;
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
      <div 
        className="bg-surface border border-border rounded-md w-full max-w-xl shadow-2xl overflow-hidden flex flex-col text-text-primary text-xs max-h-[88vh]"
      ...
      >
        ...
        <div className="p-3 sm:p-4 bg-surface-subtle border-b border-border flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-sm text-text-secondary hover:text-text-primary hover:bg-surface border border-transparent hover:border-border cursor-pointer shrink-0 ml-1"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {/* Severity Tabs */}
        <div className="p-2.5 sm:p-3 border-b border-border bg-surface-subtle/40 flex flex-wrap items-center gap-1.5 sm:gap-2">
          {(['ALL', 'CRITICAL', 'WARNING', 'INFO'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setFilter(tab)}
              className={`px-2.5 sm:px-3 py-1 rounded-xs text-[10px] sm:text-[11px] font-semibold transition-colors cursor-pointer ${
                filter === tab
          ...
          <span className="ml-auto font-mono text-[10px] sm:text-[11px] text-text-secondary">
              ...
              <div
                ...
                className={`p-3 rounded-sm border transition-colors ${
                  alert.acknowledged
                    ...
                    ? 'bg-status-blocked-tint/40 border-status-blocked-solid/50'
                    : alert.severity === 'WARNING' || alert.severity === 'HIGH RISK'
                    ? 'bg-status-highrisk-tint/40 border-status-highrisk-solid/50'
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                  ...
                  {!alert.acknowledged && (
                    <button
                      ...
                      className="self-end sm:self-start px-2.5 py-1 text-[10px] font-semibold rounded-xs bg-[#1B4B73] hover:bg-[#123A5A] text-white shrink-0 btn-press cursor-pointer shadow-xs"
                    >
                      Acknowledge
                    </button>
                  )}
